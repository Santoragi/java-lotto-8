...
public class NumberValidator {
    ...
    private void validateMinNumber(int number) {
        if(number < MIN_NUMBER) {
            throw new IllegalArgumentException("[ERROR] 로또 번호는 최소 1입니다.");
        }
    }

    private void validateMaxNumber(int number) {
        if(number > MAX_NUMBER) {
            throw new IllegalArgumentException("[ERROR] 로또 번호는 최대 45입니다.");
        }
    }
    ...
    private void validateCount(List<Integer> numbers) {
        if(numbers.size() != LOTTO_NUMBER_COUNT) {
            throw new IllegalArgumentException("[ERROR] 당첨 번호의 개수는 6개입니다.");
        }
    }
}
